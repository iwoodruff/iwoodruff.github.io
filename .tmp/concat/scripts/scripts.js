      ...
      link: function (scope, element, attrs) {
        ...
        imgLrg.bind('load', function () {
          ...
          if (!options.fullWidth) {
            ...
            imgContainer.css({ 'height': heightLrg * 0.85 });
            imgZoom.css({ 'height': heightZoom });
            target = imgContainer;
            offsetX = parseInt(target.offset().left);
            browserWindow.bind('resize', function () {
            ...
            shoe = true;  // target.bind('mousemove', function (event) {
          ...
          imgZoom.bind('load', function () {
            ...
            target.bind('mousemove', function (e) {
              if (!!shoe) {
                ...
                scope.$apply(scope.activeRotator = activeRotator);
                var x = percentageMoused * diffWidth;
                angular.element('.img-lrg').css({ 'webkitTransform': 'translate3D(' + x + 'px, 0px, 0px)' });
              }
              // scope.parallaxZoom = function (bind) {
              // if (bind) {
              console.log('hi');
              ...
              imgZoom.css({ 'webkitTransform': 'translate3D(' + imgX + 'px, ' + imgY + 'px, 0px)' });  // } else {
